...
def test():
    ...
    dex.exchange(i=0, dx=5000 * DECIMALS_1, min_dy=1,
    ...
    dex.add_liquidity(_amount0=200 * DECIMALS_1, _amount1=0 *
                      DECIMALS_0, min_token=1).run(sender=alice)
    scenario.h2("Alice :: Adding liquidity. 10")
    dex.add_liquidity(_amount0=10 * DECIMALS_1, _amount1=0 *
                      DECIMALS_0, min_token=1).run(sender=alice)
    scenario.h2("Alice :: Adding liquidity. 5000")
    dex.add_liquidity(_amount0=5000 * DECIMALS_1, _amount1=0 *
                      DECIMALS_0, min_token=1).run(sender=alice)
    scenario.h2("Alice :: Adding liquidity. 200")
    dex.add_liquidity(_amount0=200 * DECIMALS_1, _amount1=200 *
                      DECIMALS_0, min_token=1).run(sender=alice)
    scenario.h2("Alice :: Adding liquidity. 100")
    dex.add_liquidity(_amount0=100 * DECIMALS_1, _amount1=10 *
                      DECIMALS_0, min_token=1).run(sender=alice)
    scenario.h2("Alice :: Adding liquidity. 5000")
    dex.add_liquidity(_amount0=5000 * DECIMALS_1, _amount1=100 *
                      DECIMALS_0, min_token=1).run(sender=alice)

    dex.remove_liquidity(_amount=200 * DECIMALS_1,
                         min_tokens={0: 1, 1: 1}).run(sender=alice)
    dex.remove_liquidity(_amount=10 * DECIMALS_1,
                         min_tokens={0: 1, 1: 1}).run(sender=alice)
    dex.remove_liquidity(_amount=5000 * DECIMALS_1,
                         min_tokens={0: 1, 1: 1}).run(sender=alice)
    dex.remove_liquidity(_amount=400 * DECIMALS_1,
                         min_tokens={0: 1, 1: 1}).run(sender=alice)
    dex.remove_liquidity(_amount=110 * DECIMALS_1,
                         min_tokens={0: 1, 1: 1}).run(sender=alice)
    dex.remove_liquidity(_amount=4500 * DECIMALS_1,
                         min_tokens={0: 1, 1: 1}).run(sender=alice)

    dex.remove_liquidity(_amount=50_000 * DECIMALS_1,
    ...
    dex.add_liquidity(_amount0=200 * DECIMALS_0, _amount1=0 *
                      DECIMALS_1, min_token=200).run(sender=alice, valid=False)

    scenario.h2(
        "Test 6: Adding liquidity by Alice. Should fail as she is adding more liquidity than what she is approved for.")
    dex.add_liquidity(_amount0=100001 * DECIMALS_0, _amount1=200 *
                      DECIMALS_1, min_token=400).run(sender=alice, valid=False)

    scenario.h2(
        "Test 7: Adding liquidity by Alice. Should fail as she is sending tez along with the transaction.")
    dex.add_liquidity(_amount0=200 * DECIMALS_0, _amount1=0 * DECIMALS_1,
    ...
    dex.add_liquidity(_amount0=200 * DECIMALS_0, _amount1=200 *
                      DECIMALS_1, min_token=400).run(sender=alice)
    ...
    dex.remove_liquidity(_amount=400 * DECIMALS_E, min_tokens={
                         0: 1000*DECIMALS_0, 1: 1000*DECIMALS_1}).run(sender=alice, valid=False)

    scenario.h2(
        "Test 10: Removing liquidity by Alice. Should fail as she does not have sufficient LP tokens.")
    dex.remove_liquidity(_amount=401 * DECIMALS_E, min_tokens={
                         0: 200*DECIMALS_0, 1: 200*DECIMALS_1}).run(sender=alice, valid=False)

    scenario.h2(
        "Test 11: Removing liquidity by Alice. Should fail as she is sending tez along with the transaction.")
    dex.remove_liquidity(_amount=100 * DECIMALS_E, min_tokens={
                         0: 2*DECIMALS_0, 1: 2*DECIMALS_1}).run(sender=alice, amount=sp.mutez(1), valid=False)

    scenario.h2(
        "Test 12: Removing liquidity by Alice. Should succeed with correct return values.")
    dex.remove_liquidity(_amount=400 * DECIMALS_E,
                         min_tokens={0: 200*DECIMALS_0, 1: 200*DECIMALS_1}).run(sender=alice)
    ...
    dex.exchange(i=0, dx=5000 * DECIMALS_0, min_dy=5001 * DECIMALS_1, time_valid_upto=sp.timestamp_from_utc(2022,
    ...
    dex.exchange(i=0, dx=100001 * DECIMALS_0, min_dy=5001 * DECIMALS_1, time_valid_upto=sp.timestamp_from_utc(
    ...
    dex.exchange(i=0, dx=100 * DECIMALS_0, min_dy=50 * DECIMALS_1, time_valid_upto=sp.timestamp_from_utc(2022, 2, 21,
    ...
    dex.exchange(i=0, dx=5000 * DECIMALS_0, min_dy=4986 * DECIMALS_1, time_valid_upto=sp.timestamp_from_utc(
    ...
    dex.exchange(i=0, dx=1000 * DECIMALS_0, min_dy=995 * DECIMALS_1, time_valid_upto=sp.timestamp_from_utc(
        2022, 2, 21, 23, 59, 59)).run(sender=bob, now=sp.timestamp_from_utc(2022, 2, 20, 23, 59, 59))
    ...
    dex.exchange(i=1, dx=1000 * DECIMALS_1, min_dy=1001 * DECIMALS_0, time_valid_upto=sp.timestamp_from_utc(
        2022, 2, 21, 23, 59, 59)).run(sender=bob, now=sp.timestamp_from_utc(2022, 2, 20, 23, 59, 59))
    ...
    dex.remove_liquidity(_amount=100001 * DECIMALS_E, min_tokens={
                         0: 50000*DECIMALS_0, 1: 50000*DECIMALS_1}).run(sender=admin, valid=False)

    scenario.h2("Test 23: Removing all liquidity by Admin. Should succeed.")
    dex.remove_liquidity(_amount=100000 * DECIMALS_E,
                         min_tokens={0: 40000*DECIMALS_0, 1: 40000*DECIMALS_1}).run(sender=admin)
