...
def test():
    ...
    # Test 1: Initialising by Oscar. Should fail as he is not admin.
    ...
    dex.add_liquidity(_amount0 = 200 * DECIMALS_1, _amount1 = 200 * DECIMALS_0, min_token=400).run(sender=alice)
    ...
    # Test 9: Single sided claim of admin fees by Admin. Should succeed.
    ...
    # Test 8: Claiming admin fees by Oscar. Should fail as he is not admin.
    dex.admin_claim().run(sender=oscar, valid=False)

    # Test 9: Claiming admin fees by Admin. Should succeed.
    dex.admin_claim().run(sender=admin)

    # Test 1: Updating A by Oscar. Should fail as he is not admin.
    ...
    # Test 3: Updating fee by Oscar. Should fail as he is not admin.
    ...
    # Test 5: Update the lp token address by Oscar. Should fail as he is not admin.
    dex.set_lp_address(lp.address).run(sender=oscar, valid=False)

    # Test 5: Update the lp token address. Should succeed.
    dex.set_lp_address(lp.address).run(sender=admin)
