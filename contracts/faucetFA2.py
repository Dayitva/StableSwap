...
class FA2_core(sp.Contract):
    def __init__(self, config, metadata, **extra_storage):
        ...
        self.init(
            ...
            metadata = metadata,
            dripRate = sp.map(l={}, tkey=sp.TNat, tvalue=sp.TNat),
            **extra_storage
        )
    ...
    def get_balance_onchain(self, params):
        sp.trace({"Token Id Got": params.token_id})
...
class FA2_mint(FA2_core):
    ...
    @sp.entry_point
    def mint_tokens(self, token_id):
        """ Mint 1000 tokens to the sender. """
        dripAmount = sp.local("dripAmount", 1000)

        sp.if ~self.data.dripRate.contains(token_id):
            self.data.dripRate[token_id] = dripAmount.value
        sp.else:
            dripAmount.value = self.data.dripRate[token_id]
        ...
        sp.if self.data.ledger.contains(user):
            self.data.ledger[user].balance += dripAmount.value
        sp.else:
            self.data.ledger[user] = Ledger_value.make(dripAmount.value)
        sp.if ~ self.token_id_set.contains(self.data.all_tokens, token_id):
            sp.failwith("The token you want to mint doesn't exist.")
        if self.config.store_total_supply:
            self.data.total_supply[token_id] = dripAmount.value + self.data.total_supply.get(token_id, default_value = 0)
    
    @sp.entry_point
    def set_drip_amount(self, token_id, amount):
        """ Set the drip amount of a specif token_id """
        sp.verify(self.is_administrator(sp.sender), message = self.error_message.not_admin())
        sp.verify(token_id >= 0, message = "token_id < 0")
        
        sp.if ~ self.token_id_set.contains(self.data.all_tokens, token_id):
            sp.failwith("The token you want to mint doesn't exist.")

        self.data.dripRate[token_id] = amount
...
class FA2(FA2_change_metadata, FA2_token_metadata, FA2_mint, FA2_burn, FA2_administrator, FA2_pause, FA2_core):
    ...
    @sp.onchain_view()
    def get_total_supply(self, tokenId):
        sp.set_type(tokenId, sp.TNat)
        total_supply = sp.local("total_supply", 0)
        if self.config.store_total_supply:
            total_supply.value = self.data.total_supply.get(tokenId, default_value=0)
        sp.result(total_supply.value)
