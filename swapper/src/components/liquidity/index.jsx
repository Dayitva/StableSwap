...
export default function LiquidityTab() {
  ...
  return (
    <div className="bg-gray-900 border-2 border-gray-700 p-2 sm:p-4 rounded-md relative">
      <div className="flex items-center space-x-4 mb-4 border-b-2 border-gray-800">
        {tabs.map((tab, index) => (
          <span
            key={index}
            onClick={() => setSelected(tab)}
            className={`cursor-pointer border-b-2 pb-2 border-transparent hover:border-b-2 hover:border-white hover:text-white transition-all text-sm sm:text-md font-medium ${
