...
class FA12_mint_burn(FA12_core):
    ...
    def mint_tokens(self):
        self.addAddressIfNecessary(sp.sender)
        self.data.balances[sp.sender].balance += self.data.dripRate
        self.data.totalSupply += self.data.dripRate
    
    @sp.entry_point
    def set_drip_rate(self, params):
        sp.set_type(params, sp.TRecord(rate = sp.TNat))
        sp.verify(self.is_administrator(sp.sender), FA12_Error.NotAdmin)
        self.data.dripRate = params.rate
